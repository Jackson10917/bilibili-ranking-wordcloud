...
class TitleAnalyzer:
    ...
    def __init__(
        ...
        *,
        ...
        phrases = sorted(stopword_policy.title_phrases, key=lambda word: (-len(word), word))
        self._phrase_pattern = (
            re.compile("|".join(re.escape(word) for word in phrases), re.IGNORECASE)
            if phrases
            else None
        )
    ...
    def _candidate_tokens(self, title: str) -> list[str]:
        ...
        if self._phrase_pattern is not None:
            normalized = self._phrase_pattern.sub(" ", normalized)
        tokens: list[str] = []
    ...
    def analyze(self, records: Iterable[VideoRankingRecord]) -> dict[str, int]:
        return self.analyze_titles(record.title for record in records)

    def analyze_titles(self, titles: Iterable[str]) -> dict[str, int]:
        """榜单 API 与历史 CSV 共用同一分词和过滤规则。"""
        words: Counter[str] = Counter()
        for title in titles:
            for candidate in self._candidate_tokens(title):
