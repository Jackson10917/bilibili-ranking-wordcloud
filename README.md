...
CSV 使用 `utf-8-sig` 编码，可直接使用 Excel 打开。标题和 UP 主名称若以 `=`、`+`、`-`、`@` 开头，会加单引号前缀，避免电子表格把投稿内容当公式求值。再次运行不会删除已有结果；单进程在同一秒内多次运行时，新结果会在文件名中追加 `-2`、`-3` 后缀，避免覆盖（多进程并行且落在同一秒时不保证）。若标题清洗后没有可用词元，或词云生成失败（如缺少字体），则只生成 CSV，并在失败时给出警告。
