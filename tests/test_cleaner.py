...
def test_precise_campaign_filters_preserve_meaningful_words():
    from bilibili_ranker.cleaner import TitleAnalyzer, load_default_dictionary
    from bilibili_ranker.stopwords import load_stopword_policy

    load_default_dictionary()
    analyzer = TitleAnalyzer(load_stopword_policy())
    tokens = analyzer.analyze_titles(
        [
            "制作教程 魔女养成计划 全民 运动 扶持企业 原创小曲",
            "AI全民制作人 bilibilionly同人扶持计划 全网首发 空城计の小曲",
            "手机首发体验",
        ]
    )
    for word in ("制作", "计划", "全民", "扶持", "小曲", "首发"):
        assert tokens[word] == 1
    assert tokens["ai"] == 1
    assert "空城计" in tokens
    assert "同人" not in tokens


def test_optional_title_phrase_file_respects_exact_allowlist(tmp_path):
    from bilibili_ranker.cleaner import TitleAnalyzer
    from bilibili_ranker.stopwords import load_stopword_policy

    (tmp_path / "custom_stopwords.txt").write_text("", encoding="utf-8")
    (tmp_path / "allowlist.txt").write_text("testphrase", encoding="utf-8")
