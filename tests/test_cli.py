...
def test_cli_trend_compares_last_two_windows() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        # 15 份快照：上期 = d2..d8，本期 = d9..d15，d1 落在两窗之外。9 份时「前 7 期」与
        # 「最近 7 期之外的全部历史」取到同一批文件，钉不住窗口宽度，必须推到 15。
        # 每窗 7 份内容相同 → 窗内总词频 = 单份词频 × 7，排名就是单份词频的降序。
        snapshots = {"word_frequency_20240101T000000Z.csv": "词,词频\n窗口外词,99\n"}
        previous = "掉出词,6\n下降词,5\n持平词,4\n填料词,3\n上升词,2\n"
        recent = "上升词,6\n填料词,5\n持平词,4\n下降词,3\n新进词,1\n"
        ...
        with (root / TREND_CSV_NAME).open(encoding="utf-8-sig", newline="") as stream:
            rows = list(csv.DictReader(stream))
        # 精确行集即断言：窗口外词只在两窗之外的 d1 出现过，既不是掉出也不是新进，
        # 它一旦出现在表里就说明上期窗口又在跟着历史长度无限增长。
...
def test_offline_missing_history_removes_stale_derived_outputs(tmp_path):
    names = ("word_frequency_aggregate.csv", "word_frequency_trend.csv", "wordcloud_aggregate.png")
    for name in names:
